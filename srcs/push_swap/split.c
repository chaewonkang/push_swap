...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/07/06 17:24:32 by ckang             #+#    #+#             */
/*   Updated: 2021/07/06 18:13:37 by ckang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void check_med(t_stack *e, int *st, int a)
{
	if (a)
	{
		if (e->is_med && next_target(st, e->dist, e->med, 1) != INT_MAX && (e->med_pushed = 1) && e->len_b > 1)
			proceed_op(e, RB);
	...
	{
		if (e->is_med && next_target(st, e->dist, e->med, 3) != INT_MIN && (e->med_pushed = 1) && e->len_a > 1)
			proceed_op(e, RA);
		else if (e->is_med && next_target(st, e->dist, e->med, 2) == INT_MIN)
			e->med_pushed = 1;
